...
async function post(req,res,active,u,c){
  ...
  if(u==='/login'){if(hit('li:'+req.socket.remoteAddress,20,600000))return H.html(res,429,L.errPage(429,'TOO MANY REQUESTS','Slow down.'));return await WA.postLogin(req,res,b);}
  if(u==='/register'){if(hit('rg:'+req.socket.remoteAddress,10,3600000))return H.html(res,429,L.errPage(429,'TOO MANY REQUESTS','Slow down.'));return await WA.postReg(req,res,b);}
