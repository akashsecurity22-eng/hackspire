...
async function dl(req,res,user,id){
  ...
  let fp=path.join(cfg.UPLOAD_DIR,path.basename(r.file_path));
  if(!fs.existsSync(fp)){
    const tmpFp=path.join('/tmp','hackspire-uploads',path.basename(r.file_path));
    if(fs.existsSync(tmpFp))fp=tmpFp;
  }
  if(!fs.existsSync(fp))return H.html(res,404,require('./layout').errPage(404,'RESOURCE NOT FOUND','File is missing.'));
